...
fn main() -> io::Result<()> {
    ...
    let mut all_training_data: Vec<rusty_titanic::TitanicTrainingData> = Vec::new();

    for record in training_data_reader.deserialize() {
        let record: rusty_titanic::TitanicTrainingData = record?;
        all_training_data.push(record);
    }

    let split_idx = (all_training_data.len() as f32 * 0.2) as usize;

    let mut training_data_split = all_training_data.split_at_mut(split_idx);

    let mut training_data = training_data_split.0.to_vec();
    let mut test_data = training_data_split.1.to_vec();

    let parsed_test_data: Vec<f64> = parse_training_data(&test_data)?;
    let parsed_test_data_targets: Vec<_> = test_data
        .iter()
        .filter(|x| !x.ticket.contains(&"LINE"))
        .map(|x| x.survived as f64)
        .collect();
    info!("parsed_test_data_targets.len(): {}", parsed_test_data_targets.len());

    let parsed_training_data: Vec<f64> = parse_training_data(&training_data)?;
    let parsed_training_data_targets: Vec<_> = training_data
        .iter()
        .filter(|x| !x.ticket.contains(&"LINE"))
        .map(|x| x.survived as f64)
        .collect();
    info!("parsed_training_data_targets.len(): {}", parsed_training_data_targets.len());

    let targets = linalg::Vector::new(parsed_training_data_targets);

    // let mut log_mod = learning::logistic_reg::LogisticRegressor::default();
    // log_mod.train(&parsed_training_data_matrix, &targets).unwrap();
    //
    // let new_point = linalg::Matrix::new(training_data_subset.len() / 8, 8, test_data);
